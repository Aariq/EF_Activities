...
As we did in Exercise 5, we need to evaluate the output of the MCMC and will use the `coda` package to simplify these graphs and calculations. 
...
You can see within the loop that we're plotting our process model---`b0 + b1*x`---for pairs of regression parameters from the MCMC which created a distribution of models.  The reason that we use pairs of values from the posterior (i.e. rows in `var.mat`) rather than simulating values from the posterior of each parameter independently is to account for the covariance structure of the parameters.  As we saw in the pairs plot above, the covariance of the slope and intercept is considerable, and thus independent sapling of their marginal posterior distributions would lead to credible intervals that are substantially too large.
...
xpred <- 0:20  ## sequence of x values we're going to make predictions for
npred <- length(xpred)				           
...
beta <- matrix(c(b0,b1,b2,b3), nrow = 4, ncol = 1)		## put “true” regression parameters in a matrix
sigma <- 4				## define the standard deviation
x1 <- runif(n, 0, 20)
x2 <- runif(n, 0, 15)
